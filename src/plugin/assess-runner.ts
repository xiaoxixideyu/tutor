...
function parseJsonBlock(text: string): ParseResult {
  const data = extractProfileJson(text)
  if (data === null) return { ok: false, error: '未找到 JSON 代码块' }
  return { ok: true, value: data }
}

function trySchema(schema: (data: unknown) => unknown, data: unknown): ParseResult {
  try {
...
async function generateTurn(chat: AgentChat, prompt: string, parse: (text: string) => ParseResult): Promise<unknown> {
  ...
  throw new Error('tutor: 多次生成仍未通过校验，中止测评准备')
}
